...
```json
{
  ...
  "activeConversationId": "string (optional - để quản lý notification)"
}
```
...
| `activeConversationId` | string | No | ID conversation user đang xem. Nếu `activeConversationId != conversationId` → gửi notification. Nếu bằng nhau hoặc null → không gửi notification |
...
3. API tự động fetch dữ liệu từ các downstream services dựa trên context:
   - **Có context** (`SPENDING_WIDGET`, `SAVING_WIDGET`, `GOAL_WIDGET`): Gọi `/api/summary/7days` từ Finance-service
   - **Không có context**: Gọi `/api/summary/month-optimized` từ Finance-service
   - Learning Service: `lb://LEARNING-SERVICE/api/learning/ai-response`
   - Gamification Service: `lb://GAMIFICATION-SERVICE/api/challenges/summary`
4. **Guidance System (Option 1, 3, 6):**
   - Option 3: Phân tích câu hỏi để phát hiện keywords (ví dụ: "giao dịch gần đây", "tiến độ mục tiêu")
   - Option 6: Lấy template hướng dẫn phù hợp với topic
   - Option 1: Đưa guidance message vào prompt để AI lồng ghép tự nhiên
5. Dữ liệu, lịch sử và guidance được đưa vào context cho AI
6. AI xử lý và trả về JSON response
7. Response được sanitize (loại bỏ thông tin nhạy cảm như số thẻ tín dụng, SSN, từ ngữ không phù hợp)
8. Message được lưu vào database (trừ trường hợp gọi với `context` thuộc nhóm widget, xem bên dưới)
9. **Notification:** Nếu `activeConversationId != conversationId` và có lưu lịch sử → gửi FCM notification
...
**Example Request - Chat với notification handling:**
```bash
curl -X POST http://localhost:8080/api/chat/ask \
  -H "Authorization: Bearer <JWT>" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "activeConversationId": "xyz-456"
  }'
```
# → activeConversationId != conversationId → sẽ gửi notification
...
- **Quan trọng:** Luôn gửi `activeConversationId` (conversation user đang xem) để quản lý notification:
  - Nếu `activeConversationId == conversationId` → không gửi notification (user đang xem)
  - Nếu `activeConversationId != conversationId` hoặc null → gửi notification
- Các widget Tư vấn AI: gửi `context` tương ứng (`SPENDING_WIDGET`, `SAVING_WIDGET`, `GOAL_WIDGET`), không cần `question`, backend sẽ không lưu lịch sử.
...
| Service | URL | Description | Context |
|---------|-----|-------------|---------|
| Finance (7 days) | `lb://FINANCE-SERVICE/api/summary/7days` | Dữ liệu 7 ngày gần nhất | Khi có context (SPENDING_WIDGET, SAVING_WIDGET, GOAL_WIDGET) |
| Finance (month) | `lb://FINANCE-SERVICE/api/summary/month-optimized` | Dữ liệu tháng hiện tại | Khi không có context |
| Finance (daily) | `lb://FINANCE-SERVICE/api/summary/daily` | Dữ liệu 1 ngày cụ thể | Cho `/api/reports/daily` |
| Learning | `lb://LEARNING-SERVICE/api/learning/ai-response` | Hoạt động học tập | Khi có context hoặc câu hỏi liên quan |
| Gamification | `lb://GAMIFICATION-SERVICE/api/challenges/summary` | Thử thách và điểm thưởng | Khi có context SAVING_WIDGET, GOAL_WIDGET hoặc câu hỏi liên quan |
| Notification | `lb://NOTIFICATION-SERVICE/api/notifications` | Gửi FCM notification | Khi `activeConversationId != conversationId` |
...
- Finance service tự động chọn URL dựa trên context (7days cho widget, month-optimized cho chat thường)
...
10. **Guidance System (Option 1, 3, 6):** Tự động phát hiện keywords trong câu hỏi (Option 3), lấy template hướng dẫn (Option 6), và đưa vào prompt để AI lồng ghép tự nhiên (Option 1). Xem chi tiết trong `GUIDANCE_OPTION_1_3_6.md`
11. **Notification Management:** Gửi FCM notification khi `activeConversationId != conversationId` để tránh spam notification khi user đang xem conversation
12. **Finance API Selection:** Tự động chọn `/api/summary/7days` khi có context widget, `/api/summary/month-optimized` khi không có context, `/api/summary/daily` cho daily reports
...
- ✅ Thêm `activeConversationId` vào `/api/chat/ask` để quản lý notification
- ✅ Tích hợp Guidance System (Option 1, 3, 6): QuestionAnalyzer + GuidanceTemplateService
- ✅ Tự động chọn Finance API dựa trên context: `/api/summary/7days` cho widget, `/api/summary/month-optimized` cho chat thường
- ✅ ReportController sử dụng `/api/summary/daily` cho daily reports
- ✅ FCM Notification integration: Gửi notification khi `activeConversationId != conversationId`
- ✅ Cập nhật notification content: title, body (hiển thị câu hỏi), data (question, conversationId, type)
